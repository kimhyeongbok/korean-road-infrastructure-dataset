...
## Abstract
Comprehensive datasets are essential for advancing AI-driven road infrastructure solutions. However, existing resources predominantly focus on vehicles or a limited range of object categories. To bridge this gap, we introduce the Korean Road Infrastructure Dataset (KRID), a large-scale collection tailored for real-world maintenance and safety applications. KRID spans highways, national roads, and local roads in both urban and rural settings, covering 34 distinct facility types—from common elements (e.g., traffic signals, delineator posts) to specialized structures (e.g., tunnels, guardrails). Each instance is annotated with bounding boxes or polygon segmentation under strict quality-control protocols and privacy safeguards. To validate KRID’s utility, we conducted object detection and segmentation experiments using standard YOLO-based models, focusing on guardrail damage detection and traffic sign recognition. These preliminary results confirm KRID’s effectiveness in complex, safety-critical scenarios within intelligent transportation systems. Our main contributions include: (1) an extensive range of infrastructure classes beyond conventional "driving perception" datasets, (2) high-resolution, privacy-compliant annotations across diverse road conditions, and (3) open-access availability via AI Hub and GitHub. By incorporating overlooked infrastructure elements, KRID facilitates AI-driven maintenance workflows, hazard detection, and broader innovations in road safety.
